...
for (let x = 0; x < 10; x++) {
  for (let y = 0; y < 7; y++) {
    if (seededRandom() > 0.7) {
      const level = Math.floor(seededRandom() * 4) + 1;
...
for (let i = 0; i < 20; i++) {
  // Create a simple clockwise square pattern
  const movePattern = [
    [1, 0], [1, 0], [0, 1], [0, 1],  // Right, down
    [-1, 0], [-1, 0], [0, -1], [0, -1]  // Left, up
  ];
  const [dx, dy] = movePattern[i % movePattern.length];
